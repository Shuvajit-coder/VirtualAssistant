...
function Home() {
  ...
  const handleCommand = (data) => {
    ...
    //  HANDLE LIMIT FIRST
    if (type === "limit") {
      speak("I'm a bit busy right now, please try again in a few seconds.");
  ...
  useEffect(() => {
    ...
    recognition.onerror = (event) => {
      ...
      isRecognizingRef.current = false;
      setListening(false);
      // Let onend handle the restart to prevent InvalidStateError from double starts
    };
    ...
    const greeting = new SpeechSynthesisUtterance(
      `Hello ${userData.name}, what i can help you with?`,
    );
    greeting.lang = "en-IN";
  ...
  return (
    ...
    transition-transform duration-300`}
      ...
      {/* GIF */}
      {!aiText && <img src={aiImg} alt="" className="w-[120px] sm:w-[160px]" />}
      {aiText && (
